...
use tetr_ch::{
    constants::rank_col,
    model::{record::SinglePlayRecord, user::User},
};

pub fn is_supporter(user: &User) -> bool {
    if let Some(b) = user.supporter {
        b
    } else {
        false
    }
...
pub fn is_badstanding(user: &User) -> bool {
    if let Some(b) = user.badstanding {
        b
    } else {
        false
    }
...
pub fn fmt_finesse(record: SinglePlayRecord) -> String {
    ...
    format!(
        ...
        round_mid(
            finesse.perfectpieces.unwrap() as f64 / record.endcontext.piecesplaced.unwrap() as f64,
            4
        ) * 100.
    )
}
...
pub fn rank_col(rank: &str, percentile_rank: &str) -> u32 {
    let mut r = rank;
    if rank == "z" {
        r = percentile_rank;
    }
